...
OFFExporter::OFFExporter(std::vector<TriangleType *> *Triangles, std::vector<VertexType *> *Vertices,
...
void OFFExporter::WriteSurfaceData(std::string Filename)
{
    ...
    for (auto v : UsedVertices) {
        OFFFile << std::setiosflags(std::ios::fixed) << std::setprecision(15) << v->get_c(0) << " " << v->get_c(1)
                << " " << v->get_c(2) << "\n";// << "# " << v->ID << "\n";
    }
