...
Exporter::Exporter(std::vector<TriangleType *> *Triangles, std::vector<VertexType *> *Vertices,
                ...
                for (int j = 0; j < 4; j++) {   // Test node
                    ...
                    if (k == 0) {
                        if (fabs(cvalue - MaxCoords[i]) < 1e-8) {
                            TheNodes.push_back(j +
                                               1);                                 // +1 to match the numbering in the elemenent manual
                        }
                    } else {
