...
SimpleExporter::SimpleExporter(std::vector<TriangleType *> *Triangles, std::vector<VertexType *> *Vertices,
...
void SimpleExporter::WriteSurfaceData(std::string Filename, int VolumeID)
{
    ...
    for (auto v : UsedVertices) {
        SimpleFile << std::setiosflags(std::ios::fixed) << std::setprecision(15) << v->ID << " " << v->get_c(0) << " "
                   << v->get_c(1) << " " << v->get_c(2) << "\n";
    }
