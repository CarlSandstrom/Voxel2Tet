...
void OOFEMExporter::WriteVolumeData(std::string Filename)
{
    ...
    for (VertexType *v: UsedVertices) {
        for (int i=0; i<3; i++) {
            if (v->get_c(i) > MaxCoords[i]) MaxCoords[i] = v->get_c(i);
            if (v->get_c(i) < MinCoords[i]) MinCoords[i] = v->get_c(i);
        }
    }
    ...
    for (VertexType *v: UsedVertices) {
        for (int i=0; i<3; i++) {
            ...
            if (fabs(cvalue-MaxCoords[i]) < eps) MaxNodes[i].push_back(v);
            if (fabs(cvalue-MinCoords[i]) < eps) MinNodes[i].push_back(v);
        }
    }
