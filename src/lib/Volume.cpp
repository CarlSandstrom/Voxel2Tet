...
bool Volume::IsPointInside(std::array<double, 3> P)
{
    ...
    std::array<double, 3> MinPoint = {{this->Surfaces[0]->Vertices[0]->get_c(0), this->Surfaces[0]->Vertices[0]->get_c(
            1), this->Surfaces[0]->Vertices[0]->get_c(2)}};
    std::vector<VertexType *> Vertices = this->GiveVertices();

    for (VertexType *v : Vertices) {
        for (int i = 0; i < 3; i++) {
            if (v->get_c(i) < MinPoint[i]) {
                MinPoint[i] = v->get_c(i);
            }
        }
    }

    for (int i = 0; i < 3; i++) {
        MinPoint[i] = MinPoint[i] - 10.0 -
                      double(i); // The last thing here is to avoid trouble where the line Minpoint-P crosses the edge of triangles
    }
    ...
    for (Surface *s : this->Surfaces) {
        ...
        for (TriangleType *t : s->Triangles) {
            double V0[3] = {
                    t->Vertices[0]->get_c(0), t->Vertices[0]->get_c(1), t->Vertices[0]->get_c(2)
            };
            double V1[3] = {
                    t->Vertices[1]->get_c(0), t->Vertices[1]->get_c(1), t->Vertices[1]->get_c(2)
            };
            double V2[3] = {
                    t->Vertices[2]->get_c(0), t->Vertices[2]->get_c(1), t->Vertices[2]->get_c(2)
            };
...
double Volume::ComputeVolume()
{
    ...
    for (Surface *s : this->Surfaces) {
        ...
        for (TriangleType *t : s->Triangles) {
            ...
            double Vc = 1. / 6. * (v3x * (v1y * v2z - v2y * v1z) + v3y * (v2x * v1z - v1x * v2z) +
                                   v3z * (v1x * v2y - v1y * v2x)) * ContributionSign;
            V = V + Vc;
        }
    }
