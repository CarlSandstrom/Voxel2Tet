...
FC_MESH MeshManipulations::GetFlippedEdgeData(EdgeType *EdgeToFlip, EdgeType *NewEdge,
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (EdgeTriangles[0]->Vertices[i] == EdgeTriangles[1]->Vertices[j]) {
                ...
                std::array<VertexType *, 2> t0edge = {{EdgeTriangles[0]->Vertices[i], EdgeTriangles[0]->Vertices[
                                                                                              (i + increment) % 3]}};
...
FC_MESH MeshManipulations::FlipEdge(EdgeType *Edge, bool SkipIntersectionCheck)
{
    ...
    for (int i = 0; i < 2; i++) {
        NearTriangles.erase(std::remove(NearTriangles.begin(), NearTriangles.end(), EdgeTriangles[i]),
                            NearTriangles.end());
    }
...
FC_MESH MeshManipulations::CheckFlipNormal(std::vector<TriangleType *> *OldTriangles,
                                           std::array<TriangleType *, 2> NewTriangles)
{
    double MaxAngle = 0.0;
    ...
    for (TriangleType *OldTriangle : *OldTriangles) {
        ...
        for (unsigned int j = 0; j < NewTriangles.size(); j++) {
            ...
            double angle1 = std::acos(
                    OldNormal[0] * NewNormal[0] + OldNormal[1] * NewNormal[1] + OldNormal[2] * NewNormal[2]);
            double angle2 = std::acos(
                    -(OldNormal[0] * NewNormal[0] + OldNormal[1] * NewNormal[1] + OldNormal[2] * NewNormal[2]));
...
FC_MESH MeshManipulations::CollapseEdgeTest(std::vector<TriangleType *> *TrianglesToSave,
                                            std::vector<TriangleType *> *TrianglesToRemove,
                                            std::vector<TriangleType *> *NewTriangles, EdgeType *EdgeToCollapse,
                                            int RemoveVertexIndex)
{
    ...
    FC = this->CheckCoarsenChord(EdgeToCollapse, EdgeToCollapse->Vertices[RemoveVertexIndex],
                                 EdgeToCollapse->Vertices[SaveVertexIndex]);
    if (FC != FC_OK) {
        return FC;
    }
...
FC_MESH MeshManipulations::CollapseEdge(EdgeType *EdgeToCollapse, int RemoveVertexIndex, bool PerformTesting)
{
    ...
    LOG("Collapse edge %u@%p (%u, %u) by removing vertex %u\n", EdgeToCollapse->ID, EdgeToCollapse,
        EdgeToCollapse->Vertices[0]->ID,
    ...
    std::set_difference(ConnectedTriangles.begin(), ConnectedTriangles.end(),
                        TrianglesToRemove.begin(), TrianglesToRemove.end(),
                        std::inserter(TrianglesToSave, TrianglesToSave.begin()));

    std::sort(TrianglesToSave.begin(), TrianglesToSave.end(),
              SortByID<TriangleType *>); //TODO: Why cant we use SortByID in the above sort and set_difference functions?
    ...
    FC_MESH FC;
    if (PerformTesting) {
        FC = this->CollapseEdgeTest(&TrianglesToSave, &TrianglesToRemove, &NewTriangles, EdgeToCollapse,
                                    RemoveVertexIndex);
    } else {
        FC = FC_OK;
    }
    ...
    std::set_intersection(RemoveVertexEdges.begin(), RemoveVertexEdges.end(),
                          TriangleToRemoveEdges.begin(), TriangleToRemoveEdges.end(),
                          std::back_inserter(EdgesToRemove));

    // Update edges
    std::vector<EdgeType *> ConnectedEdges;
    std::set_difference(RemoveVertexEdges.begin(), RemoveVertexEdges.end(), EdgesToRemove.begin(), EdgesToRemove.end(),
                        std::back_inserter(ConnectedEdges));
    ...
    for (EdgeType *se: SaveVertexEdges) {
        if (se != EdgeToCollapse) {
            for (EdgeType *ce: ConnectedEdges) {
                ...
                //Compare
                if (((se->Vertices[0] == UpdatedConnectedEdge.Vertices[0]) &
                     (se->Vertices[1] == UpdatedConnectedEdge.Vertices[1])) |
                    ((se->Vertices[0] == UpdatedConnectedEdge.Vertices[1]) &
                     (se->Vertices[1] == UpdatedConnectedEdge.Vertices[0]))) {
                    return FC_INVALIDEDGE;
                }
...
FC_MESH MeshManipulations::CheckCoarsenNormal(std::vector<TriangleType *> *OldTriangles,
                                              std::vector<TriangleType *> *NewTriangles)
{
...
FC_MESH MeshManipulations::CheckCoarsenNormalImproved(std::vector<TriangleType *> *OldTriangles,
                                                      std::vector<TriangleType *> *TrianglesToRemove,
                                                      std::vector<TriangleType *> *NewTriangles, double &error)
{
    ...
    for (TriangleType *t : RemoveVolume) {
        ...
        double Vc = 1. / 6. *
                    (v3x * (v1y * v2z - v2y * v1z) + v3y * (v2x * v1z - v1x * v2z) + v3z * (v1x * v2y - v1y * v2x)) *
                    ContributionSign;
        V = V + Vc;
    }
...
FC_MESH MeshManipulations::CheckCoarsenChord(EdgeType *EdgeToCollapse, VertexType *RemoveVertex, VertexType *SaveVertex)
{
    ...
    if (std::find(SaveVertex->PhaseEdges.begin(), SaveVertex->PhaseEdges.end(), RemoveVertexPhaseEdge) ==
        SaveVertex->PhaseEdges.end()) {
    ...
    std::array<std::array<double, 3>, 2> Normals = {{ComputeNormalizedVector(OtherEdge.at(0), OtherEdge.at(1)),
                                                            ComputeNormalizedVector(EdgeToCollapse->Vertices.at(0),
                                                                                    EdgeToCollapse->Vertices.at(1))}};
    double MaxAngle = 0.0;

    for (int i : {0, 1}) {
        double Alpha = std::acos(
                NewNormal.at(0) * Normals[i][0] + NewNormal.at(1) * Normals[i][1] + NewNormal.at(2) * Normals[i][2]);
